...
class Equipo(models.Model):
    ...
    estado = models.IntegerField(choices=[(1, 'Nuevo'), (2, 'Usado'), (3, 'Dañado'), (4, 'Revisar')], default=1)
    ...
    def get_infoEquipo(self):
        return f'{self.homologado.nombre} - {self.serie} - {self.get_estado_display()} - {self.bodega.nombre}'
...
class EquipoInstalado(models.Model):
    ...
    def dias_instalacion(self):
        if self.fecha_instalacion and self.fecha_retiro:
            return (self.fecha_retiro - self.fecha_instalacion).days
        return None
